...
void ImProcFunctions::dehazereti(const RetinexParams &retinexParams, Imagefloat *img)
{
    img->normalizeFloatTo1();
    
    const int W = img->getWidth();
    const int H = img->getHeight();
    float dehazcombi = (0.9f * retinexParams.dehaz + 0.3f * retinexParams.str)/100.f;//0.9 0.3 arbitrary values, we can imagine another formula
    float strength = LIM01(float(dehazcombi * 0.9f));
    ...
    float depthcombi = 0.5f*retinexParams.neigh + 0.15f * (500.f - retinexParams.vart);
    float depth = -LIM01(depthcombi / 100.f); //- float(retinexParams.neigh) / 100.f;
    if (options.rtSettings.verbose) {
        std::cout << "dehaze: depth = " << depth << std::endl;
    }
